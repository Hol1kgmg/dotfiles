...
## サードパーティ tap のツールを追加したい

新しい tap を使うツール（例: `hashicorp/tap/terraform` のような形式）を追加する場合は、`nix-darwin/homebrew/taps.nix` の `homebrew.taps` にリストとして tap 名を追加してから、`brew/` または `cask/` のリストに完全修飾名で追加する：

```nix
# nix-darwin/homebrew/taps.nix
homebrew.taps = [
...
その後、再ビルドすれば tap は Homebrew により通常通り自動取得される：

```zsh
sudo darwin-rebuild switch --flake .#default --impure
```

tap を削除する場合も同様に、上記の設定から削除して再ビルドする（`onActivation.cleanup = "uninstall"` によりツール本体と tap も自動削除される）。
